...
def delete_student(db):
    ...
    if connection:
        ...
        return redirect(url_for('user', db=db))
...
# @app.route('/fess/<db>', methods=['GET', 'POST'])
# def fess(db):
#     user = session.get('user')
#     if not user:
#         return redirect(url_for('login'))
...
#             ''', fess_data)
#             connection.commit()
#             flash('Fess details updated successfully')
#             return redirect(url_for('user', db=db))
...
def fess(db):
    ...
    if connection:
        ...
        # Handling POST request to update fess
        if request.method == 'POST':
            enroll_no = request.form['enroll_no']
            fess_data = (
                request.form['fees'],
            ...
            try:
                ...
                ''', fess_data)
                connection.commit()
                flash('Fess details updated successfully', 'success')
                return redirect(url_for('user', db=db))
            except Error as e:
                flash(f"Error updating fess: {e}", 'danger')
                return redirect(url_for('fess', db=db))
...
def page_not_found(e):
    user = session.get('user')
    if not user:
        return redirect(url_for('login'))  # Corrected to point to the 'login' endpoint
    
    # Return custom 404 page for logged-in users
